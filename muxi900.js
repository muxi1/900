...
function CalcTime(dur){hrs=Math.floor(dur/1000/60/60);min=Math.floor(dur/1000/60-hrs*60);sec=Math.floor(dur/1000-min*60);mil=dur.toString().slice(-3);if (min!=0){ShowDuration=" - PS4 exploitet in : "+min+" Minute"+(min==1?"":"n")+", "+sec+" Sekunde"+(sec==1?"":"n");}else {ShowDuration=" - PS4 exploitet in: "+sec+" Sekunde"+(sec==1?"":"n");}}
...
function allset(){
 ...
 msgs.innerHTML="Ausgefuehrt! Entferne jetzt das USB-Medium!";
}

function awaitpl() {
 msgs.innerHTML="GoldHEN bereits geladen.<br>Warte auf Payload ueber Port 9020 ...";
}
...
function userland() {
 ...
 if (chain.syscall(20).low == 0) {
  alert("Webkit Exploit fehlgeschlagen. Versuche es bitte noch einmal.");
  while (1);
 }
}
...
function kernelExploit() {
 ...
 var trigger_spray = function () {
  ...
  msgs.innerHTML="USB-Medium einstecken\n und unmittelbar nach dem Verschwinden der Popup-Benachrichtigung mit X bestaetigen!";
  ...
  alert("Kernel Exploit fehlgeschlagen! Reboote die PS4 und versuche es bitte erneut.");
  p.write8(0, 0);
  return;
